/**
 * Calendar service — swappable behind an interface (ported from
 * functions/src/services/calendar.ts; `Timestamp` params → `Date`).
 *
 ...
 * delegation, impersonating each target member); otherwise a logging stub.
 */
...
export interface CalendarService {
  isFree(email: string, start: Date, end: Date): Promise<boolean>;
  createHold(opts: {
    email: string;
    title: string;
    start: Date;
  ...
  createReminder(opts: { email: string; title: string; due: Date; description?: string }): Promise<void>;
}
...
class GoogleCalendarService implements CalendarService {
  ...
  async isFree(email: string, start: Date, end: Date): Promise<boolean> {
    try {
    ...
    try {
      await this.clientFor(opts.email).events.insert({
        ...
        requestBody: {
          ...
          start: { dateTime: opts.start.toISOString(), timeZone: TZ },
          end: { dateTime: opts.end.toISOString(), timeZone: TZ },
        },
      });
    } catch (err) {
