/**
 * Email service — swappable behind an interface (ported verbatim from
 * functions/src/services/email.ts).
 *
 * Auto-selects the provider at first send: if RESEND_API_KEY is present a real
 * Resend sender is used; otherwise a logging stub (so any key-less environment
 * stays log-only with zero code change). Set RESEND_API_KEY / EMAIL_FROM in the
 * server env (Render) or server/.env locally.
 */
