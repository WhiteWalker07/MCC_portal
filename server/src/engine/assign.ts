/**
 * Assignment eligibility + selection. Ported from functions/src/engine/assign.ts
 * (pure logic over an in-memory team array; `Timestamp` → `Date`).
 *
 * Eligible = active, strikes < strikeLimit, has the required skill, and (if
 * campusStrict and the request has a campus) same campus. For at-event tasks,
 * exclude anyone busy on the calendar during the event window.
 */
...
export function campusOk(m: TeamMember, request: RequestDoc, settings: Settings): boolean {
 ...
 * at-event tasks and an optional set of emails to exclude.
 */
export async function eligibleMembers(
  ...
  let pool = team.filter(
    (m) => isBaseEligible(m, requiredSkill, request, settings) && !exclude.has(m.email)
  );

  if (atEvent && request.eventStart && request.eventEnd) {
    const start = new Date(request.eventStart);
    const end = new Date(request.eventEnd);
    const free: TeamMember[] = [];
    for (const m of pool) {
      if (await calendar.isFree(m.email, start, end)) free.push(m);
    }
    pool = free;
  }
...
export async function chooseMember(
  ...
  const pool = await eligibleMembers(pt.requiredSkill, pt.atEvent, request, settings, team, calendar);

  if (pool.length === 0) {
    return {
      member: null,
      reason: `no active member with skill "${pt.requiredSkill}"${
        settings.campusStrict && request.campus ? ` on ${request.campus} campus` : ""
      }`,
    };
  }
