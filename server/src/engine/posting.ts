 ...
 *
 * Ported from functions/src/engine/posting.ts; returns a `Date` instead of a
 * Firestore Timestamp.
 */
 ...
 */
export function findNextSlot(slots: string[], takenMs: Set<number>, nowMs: number): Date {
  ...
  return new Date(nowMs + 24 * 3600 * 1000);
}
